...
<div class="content-card">
    <div class="content-header">
        <h1>📋 My Complaints</h1>
        <p class="content-subtitle">View and manage your submitted complaints</p>

        <div class="create-report-btn">
            <a href="<?php echo URLROOT; ?>/complain" class="btn btn-success">
    ...
    <div class="filters-section">
        <div class="filter-group">
            ...
            <input type="date" id="date-filter" class="filter-input" title="Filter by complaint date">
        </div>
        <div class="filter-group">
            ...
            <table class="reports-table">
                ...
                <tbody>
                    <?php foreach ($data['complaints'] as $complaint): ?>
                        <tr>
                            <td>
                                <span class="report-code"><?php echo htmlspecialchars($complaint->complain_code); ?></span>
                            </td>
                            <td>
                                <div class="title-cell">
                                    <div class="report-title"><?php echo htmlspecialchars($complaint->subject); ?></div>
                                    <small class="report-description">
                                        <?php echo htmlspecialchars(substr($complaint->description ?? '', 0, 60)); ?>
                                        <?php if (strlen($complaint->description ?? '') > 60): ?>...<?php endif; ?>
                                    </small>
                                </div>
                            </td>
                            <td>
                                <span class="category-badge"><?php echo ucfirst($complaint->category); ?></span>
                            </td>
                            <td>
                            ...
                            <td>
                                <div class="date-info">
                                    <div><?php echo date('M d, Y', strtotime($complaint->incidentDate)); ?></div>
                                    <small>Submitted: <?php echo date('M d, Y', strtotime($complaint->created_at ?? 'now')); ?></small>
                                </div>
                            </td>
                            <td>
                                <div class="action-buttons">
                                    <a href="<?php echo URLROOT; ?>/complain/viewComplaint/<?php echo htmlspecialchars($complaint->complain_code); ?>"
                                       class="btn btn-info btn-xs" title="View Details">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    <a href="<?php echo URLROOT; ?>/complain/editComplaint/<?php echo htmlspecialchars($complaint->complain_code); ?>"
                                       class="btn btn-primary btn-xs" title="Edit Complaint">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <button type="button" class="btn btn-danger btn-xs" title="Delete Complaint"
                                            onclick="event.stopPropagation(); openDeleteModal('<?php echo htmlspecialchars($complaint->complain_code); ?>', '<?php echo htmlspecialchars(addslashes($complaint->subject)); ?>')">
    ...
    <?php else: ?>
        <div class="empty-state">
            ...
            <p>You haven't submitted any complaints yet. If you have any issues or concerns, please let us know!</p>
            <a href="<?php echo URLROOT; ?>/complain" class="btn btn-success">
        ...
        <div class="modal-body">
            <div class="warning-message">
                <p class="warning-text">
                    <strong>Are you sure you want to delete this complaint?</strong>
                </p>
                <div class="complaint-info">
                    <p><strong>Complain ID:</strong> <span id="modal-complaint-code"></span></p>
                    <p><strong>Subject:</strong> <span id="modal-complaint-title"></span></p>
                </div>
                <p class="danger-text">
                    <i class="fas fa-exclamation-circle"></i>
                    This action cannot be undone. The complaint and all associated media files will be permanently deleted.
                </p>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    //filtering
    const filters = ['date-filter', 'category-filter', 'priority-filter', 'status-filter'];
    filters.forEach(id => {
        document.getElementById(id).addEventListener('change', filterComplaints);
    });
    ...
    function filterComplaints() {
        ...
        document.querySelectorAll('.reports-table tbody tr').forEach(row => {
            ...
            const rowPriority = row.querySelector('.priority-badge').textContent.toLowerCase().trim();
            ...
            row.style.display = (matchesDate && matchesCategory && matchesPriority && matchesStatus) ? '' : 'none';
        });
    }

    // delete function
    window.openDeleteModal = function(complaintCode, title) {
        document.getElementById('modal-complaint-code').textContent = complaintCode;
        ...
        document.getElementById('deleteForm').action = '<?php echo URLROOT; ?>/complain/deleteComplaint/' + complaintCode;
    };
