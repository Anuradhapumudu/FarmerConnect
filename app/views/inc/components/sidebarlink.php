  ...
  switch(tooltip.toLowerCase()) {
    ...
    default:
      ...
      if (menuText.toLowerCase().includes('trending')) {
        ...
        pageKey = 'knowledge';
      } else if (menuText.toLowerCase().includes('marketplace')) {
        pageKey = 'marketplace';
...
function updateMainContent(page = 'home') {
  ...
  const pageContent = {
    ...
    marketplace: {
      ...
      content: `
        <p>Discover premium agricultural products and services - one of many powerful tools in your complete farming platform.</p>
        <div style="margin-top: 20px; padding: 20px; background: rgba(33, 150, 243, 0.15); border-radius: 8px; border-left: 4px solid #2196f3; backdrop-filter: blur(10px);">
          <h3 style="color: #2196f3; margin-bottom: 10px;">🛍️ Available Products:</h3>
          <ul style="margin-left: 20px; color: var(--text-secondary);">
            <li>High-quality fertilizers</li>
            <li>Premium seeds and planting materials</li>
            <li>Farming equipment and tools</li>
            <li>Agricultural machinery rental</li>
  ...
  // Special handling for marketplace page - load PHP content
  if (page === 'marketplace') {
    // Show loading state
    mainContent.innerHTML = '<div style="text-align: center; padding: 50px;"><h2>Loading Marketplace...</h2></div>';
    
    // Fetch the marketplace page content from PHP
    fetch(window.location.origin + window.location.pathname.replace('/public/index.php', '') + '/farmer/marketplace')
      .then(response => response.text())
      .then(html => {
        // Extract just the main content from the response
        const parser = new DOMParser();
      ...
      .catch(error => {
        ...
        mainContent.innerHTML = `
          <h1>${content.title}</h1>
          <p>Error loading marketplace content. Please try again later.</p>
          ${content.content}
        `;
      });
    return;
